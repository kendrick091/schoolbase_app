...
let takeAttendance = document.getElementById('takeAttendance');
...
takeAttendance.addEventListener('click', ()=>{
    tog(registerFormDiv)
})
...
request.onerror = () =>{
    console.log('Error opening attendance database')
}
...
document.getElementById('submitAttendance').addEventListener('click', function () {
    ...
    studentsRequest.onsuccess = function (event) {
        ...
        // Step 2: Proceed to save attendance
        ...
        checkboxes.forEach((checkbox) => {
            ...
            attendanceStore.add(attendanceRecord);
        });

        attendanceTx.oncomplete = () => {
            alert("Attendance recorded with students ID: " + cursor.value.sessionID);
        };

        attendanceTx.onerror = () => {
...
function renderCombinedAttendance() {
    const tableBody = document.querySelector('#attendanceCombined tbody');
    tableBody.innerHTML = '';
    ...
    // Step 1: Collect today's attendance
    attendanceStore.openCursor().onsuccess = function(event) {
        const cursor = event.target.result;
        if (cursor) {
